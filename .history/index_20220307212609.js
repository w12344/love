...
let blockTop = "50%";
let blockLeft = "50%";
let blockMargin = "-20px 0 0 -20px";
block.setAttribute("style", `top:${blockTop}`);
block.setAttribute("style", `left:${blockLeft}`);
block.setAttribute("style", `margin:${blockMargin}`);
...
function Next() {
    ...
    console.log(block, '++++++++++++');
    block.style.visibility = "visible!important"; //鍗囩┖鍔ㄧ敾鍓嶅厑璁稿彲瑙�
    let cssText = "visibility:visible!important;";
    block.setAttribute("style", cssText);
    //2.绉诲姩鏂瑰潡鍒版寚瀹氱殑浣嶇疆-鍗虫槸绉诲姩銆愯嚜韬腑蹇冦€戝埌鐩爣浣嶇疆
    let left = block_left + 40 * offset_pitn["block" + index][0] + "px";
    let top = block_top - 40 * offset_pitn["block" + index][1] + "px";
    block.setAttribute("style", `left:${left}`);
    block.setAttribute("style", `top:${top}`);
    for (let i = 0; i < block.children.length; i++) {
        // block.children[1].innerText = index;    //缂栧彿渚夸簬璋冭瘯
        let childrenLeft = blk_pitn["block" + index][i][0] * -40 + "px";
        let childrenTop = blk_pitn["block" + index][i][1] * -40 + "px";
        block.children[i].setAttribute("style", `left:${childrenLeft}`);
        block.children[i].setAttribute("style", `top:${childrenTop}`);
    }
